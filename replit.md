...
-   **Admin Task Detail View:** Admin users see a management-focused task detail view (`AdminTaskDetailPage.tsx`) at `/tasks/:id` instead of the technician completion view. Features: collapsible Notes section with add/edit/delete, time entry edit/delete in History, right sidebar with actions (Log Time, Add Note, Photos/Docs, Scan), admin bar with Edit/Delete, and back navigation. The standalone page hides the fullscreen toggle since it's always fullscreen. **Mobile-responsive:** On screens < 768px, the layout switches from a two-panel side-by-side view to a single-column stacked layout. The meta grid uses 2 columns instead of 4, the sidebar content stacks below the main content, and the admin bar in the main content is hidden (since sidebar actions are visible below). Actions use a 2x2 grid, and Edit/Delete buttons sit side-by-side on mobile. Routes: `PATCH /api/time-entries/:id`, `DELETE /api/time-entries/:id`, `GET/POST/PATCH/DELETE /api/task-notes/*`.
